...
<div class="mb-6">
    ...
    <p class="text-gray-300">Creez un nouveau compte pour un receptionniste ou un coach</p>
</div>
    ...
    <form method="POST" action="{{ route('admin.staff.store') }}">
        ...
        <div class="mb-6">
            ...
            <input type="email" name="email" value="{{ old('email') }}" 
                   ...
                   required>
            <p class="mt-1 text-xs text-gray-500">Cet email sera utilise pour la connexion</p>
            @error('email')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            <label class="block text-gray-800 font-semibold mb-2"> Role</label>
            <select name="role" class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none transition" required>
                ...
                <option value="receptionist" {{ old('role') == 'receptionist' ? 'selected' : '' }}> Receptionniste</option>
        ...
        <div class="mb-6">
            ...
            <input type="password" name="password" 
                   ...
                   required>
            <p class="mt-1 text-xs text-gray-500">Le mot de passe sera utilise pour la connexion</p>
            @error('password')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
