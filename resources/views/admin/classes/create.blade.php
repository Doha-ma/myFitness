...
<div class="mb-8">
    <div class="flex justify-between items-center">
        <h2 class="text-4xl font-bold text-white mb-2">Creer un Cours</h2>
    ...
    <form method="POST" action="{{ route('admin.classes.store') }}" class="space-y-6">
        ...
        <div>
            ...
            <textarea id="description" 
                      ...
                      rows="4"
                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white text-gray-900"
                      placeholder="Decrivez le cours (optionnel)">{{ old('description') }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex justify-end gap-4">
            <a href="{{ route('admin.classes.index') }}" class="px-6 py-2 border border-gray-300 text-gray-300 rounded-lg hover:bg-gray-700 transition">
                Annuler
            </a>
            <button type="submit" class="btn-primary text-white px-6 py-2 rounded-lg hover:shadow-lg transition">
                <i class="fas fa-save me-2"></i> Creer le cours
            </button>
        </div>
    </form>
</div>
